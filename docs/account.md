...
There are two ways of creating an account on ScolCoin Chain: creating a key in a web wallet and creating a key via `eth-cli`. Please make sure you backup your mnemonic.
...
Your wallet will be unlocked and you can get your address here (for this example it is `tscol14m2gcdjq7aqkdtu2m9qrqrl8eevzpqfj9xc0uu`):
...
```bash
$  ./eth-cli account bnc1wwgakqy32m7vdnlf00pctf9hnaak37eh7wkmqa --trust-node --chain-id SCOL-Chain-Tigris --node https://dataseed5.defibit.io:443
```
Example on **testnet**:

```bash
$  ./eth-cli account bnc1wwgakqy32m7vdnlf00pctf9hnaak37eh7wkmqa --trust-node --chain-id=SCOL-Chain-Ganges --node=data-seed-pre-2-s1.scolcoin.com:80
{
