...
- [ ] https://docs.scolcoin.com/walletconnect.html
...
## Wallets with WalletConnect Support for ScolCoin Chain

* List of Wallets Supporting WalletConnect on ScolCoin Chain
...
The ScolCoin Chain Web Wallet supports connecting with external wallet providers via the [WalletConnect protocol](https://docs.walletconnect.org/tech-spec).
...
In order for this to work, some modifications to the standard WalletConnect protocol are used in the ScolCoin Chain wallet's implementation.

See the list of wallets which support WalletConnect on ScolCoin Chain [here](../../wallets/walletconnect-support.md)
...
* Typically `sendTransaction` is used with Ethereum transaction parameters in WalletConnect dApp integrations. But in ScolCoin Chain's case, instead of invoking `sendTransaction` in the WalletConnect flow, the new [`sendCustomRequest`](https://docs.walletconnect.org/client-sdk#send-custom-request) call is used instead with a method called `scol_sign` (see below).
...
Receipt of the `scol_tx_confirmation` should be confirmed by the app with `approveRequest` as per the WalletConnect protocol flow.
