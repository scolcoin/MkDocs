...
```
AtomicSwapCoinsAccAddr = sdk.AccAddress(crypto.AddressHash([]byte("ScolCoin ChainAtomicSwapCoins")))
```
The account for mainnet is: **scol1wxeplyw7x8aahy93w96yhwm7xcq3ke4f8ge93u** and the account for testnet is: **tscol1wxeplyw7x8aahy93w96yhwm7xcq3ke4ffasp3d**. Once the swap is claimed or refunded, the fund will be transfered from the purely-code-controlled escrow account to client accounts.


### Hash Timer Locked Transfer

Hash Timer Locked Transfer (HTLT) is a new transaction type on ScolCoin Chain, to serve as HSCOL in the first step of Atomic Swap,
...
| ExpectedIncome | string | Expected income from swap counter party, example: "100:SCOL" or "100:SCOL,10000:BTCB-1DE" The amount needs to be bumped by e^8 | No |
...
Command line
```shell
./eth-cli token HTLT --recipient-addr <recipient-addr> --amount 100:SCOL --expected-income <expectedIncome> --height-span <heightSpan> --from <from-addr> --chain-id SCOL-Chain-Ganges --trust-node --node http://data-seed-pre-0-s3.scolcoin.com:80
```

Javascript
```javascript
  const client = new BncClient("https://testnet-dex.scolcoin.com")
...
2. Swap from ScolCoin Chain to Ethereum

* Clients send HTLT on ScolCoin Chain on *testnet*:

Command line:
```shell
./eth-cli token HTLT --from <from-addr> --chain-id SCOL-Chain-Ganges  --height-span <heightSpan> --amount <amount> --expected-income <expectedIncome> --recipient-addr <deputy-bep2-addr>  --recipient-other-chain <client ethereum address>  --cross-chain --trust-node --node http://data-seed-pre-0-s3.scolcoin.com:80
```
...
| Amount | Coins | The swapped out amount BEP2 tokens, example: "100:SCOL" or "100:SCOL,10000:BTCB-1DE" | No |
...
Command line:
```shell
./eth-cli token deposit --swap-id <swapID>  --amount 10000:TEST-599 --from <from-key> --chain-id SCOL-Chain-Ganges --trust-node --node http://data-seed-pre-0-s3.scolcoin.com:80
```
...
Command line:
```shell
./eth-cli token claim --swap-id  <swapID> --random-number <random-number> --from <from-key> --chain-id SCOL-Chain-Ganges --trust-node --node http://data-seed-pre-0-s3.scolcoin.com:80
```
...
Command line:
```shell
./eth-cli token refund --swap-id <swapID> --from <from-key>  --chain-id SCOL-Chain-Ganges --trust-node --node http://data-seed-pre-0-s3.scolcoin.com:80
```
...
* On testnet:
```
./eth-cli token query-swap --swap-id <swapID> --chain-id SCOL-Chain-Ganges --trust-node --node http://data-seed-pre-0-s3.scolcoin.com:80
```
Expected output
```json
{
...
* On testnet:
```
./eth-cli token query-swapIDs-by-recipient  --recipient-addr <address> --chain-id SCOL-Chain-Ganges --trust-node --node http://data-seed-pre-0-s3.scolcoin.com:80
```
...
* On testnet:
```
./eth-cli token query-swapIDs-by-creator  --creator-addr <address> --chain-id SCOL-Chain-Ganges --trust-node --node http://data-seed-pre-0-s3.scolcoin.com:80
```
...
-- | -- | -- | --
HTLT | N/A |  0.000375 SCOL | Y
depositHTLT | N/A |  0.000375 SCOL | Y
claimHTLT | N/A |  0.000375 SCOL | Y
refundHTLT | N/A |  0.000375 SCOL | Y
...
* Its corresponding address on testnet is: `tscol1pk45lc2k7lmf0pnfa59l0uhwrvpk8shsema7gr`on ScolCoin Chain and `0xD93395B2771914E1679155F3EA58C41d89D96098` on Ethereum testnet

### Swap Tokens from Ethereum to ScolCoin Chain
...
#### 3. Deputy Call HTLT on ScolCoin Chain
Then, Deputy will send `HTLT` transaction [here](https://testnet-explorer.scolcoin.com/tx/99CBC2896F0CF14DDAB0684BDA0A3E9FF2271056E68EC3559AB7FB24E0EE97DE)

#### 4. Claim HTLT on ScolCoin Chain

* Get the `swapID` on ScolCoin Chain

```
./eth-cli token query-swapIDs-by-recipient  --recipient-addr tscol1cs0j4p0p6d3fvd77zg3qzlwwgmrv3e9e63423w --chain-id SCOL-Chain-Ganges --trust-node --node http://data-seed-pre-0-s3.scolcoin.com:80
[
...
|sender | string  | deputy bep2 address | tscol1pk45lc2k7lmf0pnfa59l0uhwrvpk8shsema7gr |
...
* Send claim transaction on ScolCoin Chain

```
./eth-cli token claim --swap-id  12aacc3bdc2cef97e8e45cc9b409796df57904a4e9c76863ad8420ff75f13128  --random-number <random-number> --from <from-key>  --chain-id SCOL-Chain-Ganges --trust-node --node http://data-seed-pre-0-s3.scolcoin.com:80
```

Example of `claim` tx on [testnet](https://testnet-dex.scolcoin.com/api/v1/tx/6BA714E6D107F1D9634DDC159F560A1FB61393B8E15723EFD70B9EA8B0B1AA9A?format=json)
...
This is a javascript implementation for client app to swap [PPC](https://ropsten.etherscan.io/address/0xd93395b2771914e1679155f3ea58c41d89d96098) to [PPC-00A](https://testnet-explorer.scolcoin.com/asset/PPC-00A) with deputy.
...
### Swap Tokens from ScolCoin Chain to Ethereum
![image-20190918193910521](../../assets/bnc2eth.png)

#### 1. Send `HTLT` Transaction from ScolCoin Chain
...
Example is [here](https://testnet-explorer.scolcoin.com/tx/9ECECE9E0F08EE78583CFA37FD4C3F03521289F0F229A612886B8B21B9C62D7F)

Then, you can query the the swap by `SwapID`:

```
./eth-cli token query-swap --swap-id f85dd907df0a5897927b949c0f9e2563d453ba698ff9941fed1ce91f8057afc2 --chain-id SCOL-Chain-Ganges --trust-node --node http://data-seed-pre-0-s3.scolcoin.com:80
```
...
- `_randomNumberHash` must equal to the randomNumberHash in client HTLT transaction on ScolCoin Chain
- `_recipientAddr` must equal to client ethereum wallet address
- `_timestamp` must equal to the timestamp in client HTLT transaction on ScolCoin Chain
...
#### 5. Deputy Claim on ScolCoin Chain

`Claim HTLT` transaction from **Deputy** is [sent](https://testnet-explorer.scolcoin.com/tx/8C616DEFD2EAA41E13D2DC4844B218DFF8CFE24B4C7A693AAD700381B5FF7B48) afterwards:

#### 6. Demo for Client APP: swap bep2 to erc20

This is a javascript implementation for client app to swap  [PPC-00A](https://testnet-explorer.scolcoin.com/asset/PPC-00A) to [PPC](https://ropsten.etherscan.io/address/0xd93395b2771914e1679155f3ea58c41d89d96098) with deputy.
